...
dtrain = lgb.Dataset(as.matrix(df_train), label = label, free_raw_data = F)
...
obj.fun = smoof::makeSingleObjectiveFunction(
  ...
  par.set = makeParamSet(
    ...
    makeIntegerParam("num_leaves", lower = 5, upper = 40L),
    ...
    makeNumericParam("feature_fraction", lower = 0.4, upper = 0.95)
  ),
  minimize = TRUE
)
